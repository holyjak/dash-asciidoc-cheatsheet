...
| [[quotes]]"`double curved quotes`" | `+"`double curved quotes`"+`
| '`single curved quotes`' | `+'`single curved quotes`'+`
| [[apostrophes]]apostrophes`' vs x' | `+apostrophes`' vs x'+`
|===
...
| [[source code]]source code | `----`; attributes: `[source,ruby]`
...
| [[open block]]open^1^    | `--` An anonymous container; can masquerade as any other by naming it: `+[source\|...]+`
